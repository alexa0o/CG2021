...
public:
    Fireball() = default;
    Fireball(MetaObject* meta, vec3 forward, vec3 position): Forward(forward), Position(position) {
        this->meta = meta;
        ModelMatrix = translate(mat4(), forward * 5.0f);
    }

    bool Update(float dt) {
        Position += Forward * kSpeed * dt;
        ModelMatrix = translate(mat4(), Position) * meta->Scale;
        return std::sqrt(std::pow(Position[0], 2) + std::pow(Position[1], 2) + std::pow(Position[2], 2)) <= 7;
    }
...
void load(std::vector<std::unique_ptr<Enemy>>& enemies,
    ...
    for (auto i = 0; i < bsize; ++i) {
        ...
        balls.emplace_back(std::make_unique<Fireball>(metas.second, forw, pos));
    }
}
...
int main( void )
{
    ...
    MetaObject MetaEnemy("cube.obj");
    MetaEnemy.Textures["Texture"] = loadDDS("uvmap.DDS");
	...
	do{
        ...
        if (mouseState == GLFW_RELEASE && currMouseState == GLFW_PRESS) {
            balls.emplace_back(std::make_unique<Fireball>(&MetaBall, getForward(), getForward()));
        }
        ...
        glUniform1f(MetaBall.Values["itime"], time * 0.2);
